...
def main():
    ...
    logger = setup_logger(settings.logging.level, settings.logging.log_dir, "daily_basic_app.log")

    logger.info("[流程] 每日指标数据下载开始，读取配置成功，准备初始化数据库连接与建库建表")
    ...
    with SessionFactory() as session:  # type: Session
        ...
        logger.info("[流程] 启动每日指标增量模式：按库内最大交易日续拉，默认起点=%s", settings.ingest.start_date)
        service.ingest_incremental(start_date=settings.ingest.start_date)
        session.commit()
        logger.info("[流程] 全市场每日指标数据拉取与写入完成")
