...
from src.services.daily_basic_ingest_service import DailyBasicIngestService


def main():
    """全量每日指标历史数据下载主函数"""
    settings = load_settings()
    logger = setup_logger(settings.logging.level, settings.logging.log_dir, "full_daily_basic_ingest.log")

    logger.info("[流程] 全量每日指标历史数据下载开始，读取配置成功，准备初始化数据库连接与建库建表")
    ...
    with SessionFactory() as session:  # type: Session
        ...
        service = DailyBasicIngestService(ts_client=ts_client, repo=repo, logger=logger)

        # 获取命令行参数中的开始日期，默认为配置文件中的值
        start_date = sys.argv[1] if len(sys.argv) > 1 else settings.ingest.start_date
        end_date = sys.argv[2] if len(sys.argv) > 2 else None
        
        logger.info("[流程] 启动每日指标全量模式：强制从指定日期开始下载，起始日期=%s，结束日期=%s", start_date, end_date or "今天")
        logger.info("[流程] 警告：全量模式会重新下载所有每日指标数据，可能与现有数据产生重复，但数据库使用upsert避免重复插入")
        
        # 强制使用全量下载
        service.ingest_all_from_to(start_date=start_date, end_date=end_date)
        session.commit()
        logger.info("[流程] 全量每日指标历史数据下载与写入完成")
