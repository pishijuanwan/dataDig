...
class DailyBasicIngestService:
    ...
    def ingest_all_from_to(self, start_date: str, end_date: Optional[str] = None):
        ...
        if self._logger:
            self._logger.info("[流程] 开始全市场每日指标拉取，起始=%s，结束=%s", start, end)
        ...
        if self._logger:
            self._logger.info("[流程] 交易日总数=%s，将逐日拉取每日指标数据", len(trade_dates))

        # 3) 逐交易日获取全量每日指标（按日期，两市所有可交易股票）
        for idx, trade_date in enumerate(trade_dates, 1):
            if self._logger:
                self._logger.info("[流程] (%s/%s) 拉取交易日=%s 的全市场每日指标", idx, len(trade_dates), trade_date)
            df = self._ts.query_daily_basic(trade_date=trade_date)
            if df is None or df.empty:
                if self._logger:
                    self._logger.info("[流程] 交易日=%s 无每日指标数据，跳过", trade_date)
                continue
            self._repo.upsert_daily_basic(df)
            if self._logger:
                self._logger.info("[流程] 交易日=%s 每日指标写入完成，记录数=%s", trade_date, len(df))

    def ingest_incremental(self, start_date: str):
        """基于库内最大交易日增量拉取（包含空库场景）"""
        start = self._normalize_date(start_date)
        max_date = self._repo.get_max_trade_date_basic()
        if max_date is None:
            if self._logger:
                self._logger.info("[流程] 库内无每日指标历史数据，首次全量自%s开始拉取", start)
            self.ingest_all_from_to(start_date=start)
            return
        
        # 获取从库内最大交易日之后的所有交易日
        if self._logger:
            self._logger.info("[流程] 每日指标增量模式：从库内最大交易日(%s)的下一个交易日起拉取", max_date)
        ...
        if not trade_dates:
            if self._logger:
                self._logger.info("[流程] 无需增量：最新每日指标交易日已在库中")
            return
            
        if self._logger:
            self._logger.info("[流程] 需要增量拉取每日指标交易日数量=%s", len(trade_dates))
            
        for idx, trade_date in enumerate(trade_dates, 1):
            if self._logger:
                self._logger.info("[流程] (每日指标增量 %s/%s) 拉取交易日=%s 的全市场每日指标", idx, len(trade_dates), trade_date)
            df = self._ts.query_daily_basic(trade_date=trade_date)
            if df is None or df.empty:
                if self._logger:
                    self._logger.info("[流程] 交易日=%s 无每日指标数据，跳过", trade_date)
